...
$$ \left\{  \{\pi_k^z, \pi_k^s\}_{k=1}^{K_+}, \alpha^\theta, \sigma^\theta  \right\} $$
...
\begin{align}
	...
	=	& \label{eq:likelihood} \prod_t \prod_{n} P\left(x_{nt}^*|\{\epsilon_{nkt},z_{nk},s_{nk}, \theta_{nk},\Lambda_{k}  \}_{k=1}^{K_+}\right) \\
		...
		P(\sigma^\theta),
\end{align}
where line (\ref{eq:likelihood}) is the likelihood, line (\ref{eq:prior}) is the prior and line (\ref{eq:hprior}) is the hyper prior.
